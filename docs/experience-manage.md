...
<span class="status-badge">默认仅当前浏览器 · 明确确认后才公开</span>

# 分享匿名面经

<div class="owner-notice" role="note">
  <strong>这是题库主人的发布流程。</strong>
  <span>网页访客不能直接修改当前仓库。请先创建自己的题库，或在获得当前仓库写入权限后再使用 Pages CMS。</span>
</div>

“面试记录”里的公司、进度和复盘始终先保存在当前浏览器。把分享计划改成“准备匿名分享”也不会自动上传；它只会出现一个整理入口，避免误操作泄露私人求职数据。
...
| 模式 | 保存位置 | 谁能看到 |
| --- | --- | --- |
| 仅当前浏览器 | 当前浏览器的本地存储 | 能使用同一浏览器资料或同源站点脚本的用户 |
| 准备匿名分享 | 仍在当前浏览器；只增加整理入口 | 与上面相同，仍未上传 |
| 已公开 | 自己的 GitHub 仓库和公开网页 | 互联网访问者 |

## 安全发布四步

1. 在“面试记录”中把某个流程的分享计划设为“准备匿名分享”，保存后点击“整理公开面经”。
...
把浏览器中的分享计划改回“仅当前浏览器”，只会关闭本地的公开整理入口，不会改动 GitHub 上已经发布的文章。要下线文章，请在 Pages CMS 的“匿名公开面经”中找到它，关闭“在公开网站显示这篇面经”并保存；如果还要从 Public 仓库当前版本中移除源文件，请直接删除这条面经。Git 历史仍可能保留旧版本，因此敏感内容不应先发布再撤回。
...
<a class="primary-button cms-button" href="https://app.pagescms.org/" target="_blank" rel="noopener noreferrer">题库主人：打开 Pages CMS 发布 ↗</a>
<a class="secondary-button" href="{{ '/start/' | relative_url }}">还没有题库？先创建自己的</a>
