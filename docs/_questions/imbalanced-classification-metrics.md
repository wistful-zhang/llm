...
准确率在 1% 正例任务中可能被“全预测为负”轻易做高。F1 忽略真负例且默认 Precision、Recall 同等重要，需要不同权重时可用 $$F_\beta$$。ROC-AUC 和 PR-AUC 都是排序指标，不能说明某个生产阈值下的实际成本；不同数据基准正例率变化时，Precision 和 PR 曲线也会变化。
