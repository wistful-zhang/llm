...
1. **先给结论：** MCP 用 Host 管用户与安全策略、Client 维护连接、Server 暴露资源和工具，但协议本身不替代授权。
2. **再讲关键机制：** 先画清 Host—Client—Server 边界，再讲能力协商、工具发现、参数调用和结果回传。
3. **主动说取舍：** 标准化接入降低集成成本，同时扩大供应链与提示注入面，Server 返回内容始终是不可信输入。
4. **最后落到项目：** 落到企业数据连接器，衡量握手成功率、工具失败率、越权拦截数和版本兼容率；随后停顿。

**60 秒口述示例：**

> 我会先划边界：Host 承担用户交互、授权和策略，Client 与某个 MCP Server 建立会话，Server 只声明并执行资源、提示或工具能力。调用前会做能力协商，但这不等于业务授权。取舍是接入更统一，攻击面也更大。项目里我会做最小权限和结果隔离，并看握手成功率、工具失败率、越权拦截数和协议版本兼容率。
