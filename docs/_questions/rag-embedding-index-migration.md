...
1. **先给结论**：先说新旧 Embedding 空间通常不兼容，不能在原索引里直接混写；停顿后给出蓝绿迁移主线。
2. **再讲关键机制**：按数据快照、V2 全量回填、变更双写、完整性校验、影子双读、原子切换和回滚回答。
3. **主动说取舍**：指出双索引最安全但占双份存储和计算，原地加 Named Vector 更省运维却耦合 Schema 与回收流程。
4. **最后落到项目**：用不可变版本和稳定文档 ID 落地，监控回填覆盖率、双写延迟、Recall、答案质量、P95 与回滚时间。

**60 秒口述示例：**

> 我的结论是，新旧 Embedding 即使维度相同也不能假设向量可比，所以我会建 V2 蓝绿索引，不在 V1 中混用。这里停一下：先冻结源数据快照并全量回填，期间通过变更日志双写新增、修改和删除；覆盖率与质量过门后做影子双读，再用 Alias 原子切换。项目里监控回填覆盖率、双写积压、Recall@K、答案胜率、P95 延迟和回滚 RTO。
