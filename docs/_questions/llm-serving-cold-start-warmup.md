...
1. **先给结论**：先说冷启动由制品下载、权重加载、CUDA 初始化、编译/图捕获和首次分配共同造成；列完停顿。
2. **再讲关键机制**：区分进程存活、模型加载完成、代表性请求预热成功和真正可接流量。
3. **主动说取舍**：指出预热减少首请求抖动，却拉长发布和扩容时间，也会占用真实 GPU 容量。
4. **最后落到项目**：分阶段打点并用真实形状预热，监控加载 P95、预热失败、首请求延迟、扩容 RTO 与空闲成本。

**60 秒口述示例：**

> 我的结论是，冷启动不只是下载权重，还包括反序列化、CUDA 上下文、显存池、Kernel 编译与图捕获。这里停一下，再区分 Liveness 和 Readiness：进程活着不能接流量，必须在模型加载并完成代表性长度与 Batch 预热后才就绪。取舍是预热越全面，扩容越慢。项目里我分阶段计时，监控加载 P95、预热失败率、首请求 TTFT、扩容 RTO 和预留副本成本。
