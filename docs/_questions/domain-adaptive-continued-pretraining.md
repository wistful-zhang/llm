...
1. **先给结论：** 领域继续预训练用于吸收领域语言与知识，SFT 用于学习任务格式和行为，通常先 DAPT 再 SFT。
2. **再讲关键机制：** 说明 DAPT 仍用自监督 next-token loss，SFT 用指令输入输出并做选择性 loss mask。
3. **主动说取舍：** DAPT 数据量大且可能遗忘通用能力，SFT 更省算但难注入大量隐性知识。
4. **最后落到项目：** 用领域 PPL、任务准确率、通用能力掉点和训练 token 成本决定是否需要两阶段；讲完停。

**60 秒口述示例：**

> 我会先按目标区分：模型不懂领域术语和分布时做继续预训练；已经懂知识但不会按业务格式回答时做 SFT。常见顺序是 DAPT 后再 SFT，因为前者改表示，后者塑行为。取舍是 DAPT 成本高且可能遗忘。项目里我会先测领域 PPL 和闭卷知识，再报告业务任务准确率、通用基准掉点、训练 token 与总成本。
