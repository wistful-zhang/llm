...
$$\gamma$$ 控制远期奖励折扣，$$\lambda$$ 控制使用多长的 TD 信息：$$\lambda=0$$ 接近一步 TD，方差较低但更依赖 Value 准确性；$$\lambda\to1$$ 接近 Monte Carlo 回报减基线，通常偏差更低、方差更高。
...
2. **$$\lambda=1$$ 就一定无偏吗？** 只有在相应终止、折扣和 Bootstrap 条件下才接近 Monte Carlo 估计；$$\gamma<1$$、截断和错误 Mask 仍会引入偏差。
