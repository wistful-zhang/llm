...
NCCL Hang 排查先找第一个不一致：Rank 是否提前退出、Collective 顺序或张量大小是否不同、前序 CUDA 是否报错、网络链路是否异常。按应用、进程组、NCCL、网络、硬件逐层缩小，并对齐各 Rank 的 Step/Collective 序号；调大 timeout 只能掩盖问题。

**可以这样答：**

> NCCL 超时不是先调大 timeout，而要找第一个不一致：是否有 Rank 提前退出、Collective 顺序或张量大小不同、网络链路异常，或者前一个 CUDA Kernel 已经报错。按应用、进程组、NCCL、网络与硬件逐层排。不过，全量日志有用但会产生巨大 I/O，所以先在最小规模打开定向调试。定位问题时要对齐各 Rank 的 Step 和 Collective 序号，记录首个异常 Rank、P99 通信时延、错误计数、自动重启次数和最小用例复现率。
