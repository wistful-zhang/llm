...
1. **先给结论**：先说 NCCL Hang 多半是参与者、调用顺序、张量规模、网络或前序 CUDA 错误不一致；停顿后按层排查。
2. **再讲关键机制**：从 Rank 生命周期、Collective 匹配、拓扑与异步 CUDA 错误解释为何表现为超时。
3. **主动说取舍**：指出增大超时只能掩盖慢节点，开启全量调试日志又会放大 I/O 干扰。
4. **最后落到项目**：最小规模复现并收集各 Rank 时间线，监控首个异常 Rank、Collective 延迟、重启次数和复现率。

**60 秒口述示例：**

> 我的结论是，NCCL 超时不是先调大 timeout，而要找第一个不一致：是否有 Rank 提前退出、Collective 顺序或张量大小不同、网络链路异常，或者前一个 CUDA Kernel 已经报错。这里停一下，再按应用、进程组、NCCL、网络与硬件逐层排。取舍是全量日志有用但会产生巨大 I/O，所以先在最小规模打开定向调试。项目里我会对齐各 Rank 的 Step 和 Collective 序号，记录首个异常 Rank、P99 通信时延、错误计数、自动重启次数和最小用例复现率。
