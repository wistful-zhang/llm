...
1. **先给结论：** 弹性训练要把 worker 视为可替换成员，以 rendezvous 重建进程组，并从一致 Checkpoint 恢复全局训练状态。
2. **再讲关键机制：** 讲清成员变化触发全体重启或重组、rank 重新分配、数据重分片和全局 batch 语义。
3. **主动说取舍：** 弹性提高集群利用率，却可能改变有效 batch、学习率和样本顺序，严格复现更难。
4. **最后落到项目：** 故障注入后检查 loss 连续、重复样本、恢复时长、有效 GPU 利用率和完成率；随后停。

**60 秒口述示例：**

> 我会先说明弹性不等于进程原地续跑。worker 故障或规模变化后，通过 rendezvous 形成新进程组，rank 可能重新编号，再从一致 Checkpoint 恢复模型、优化器和数据游标。若 world size 改变，还要明确全局 batch 和学习率是否保持。项目里我会注入 worker 故障，监控恢复 RTO、loss 跳变、样本重复率、有效 GPU 利用率和整次任务完成率。
