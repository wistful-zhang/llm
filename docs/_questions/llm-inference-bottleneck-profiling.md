...
1. **先给结论**：先说必须分 Prefill 与 Decode，再判断算力、带宽、容量或调度瓶颈；停顿后给证据。
2. **再讲关键机制**：用算术强度与 Roofline 解释 Kernel 上限，并把端到端等待拆成队列、计算和通信。
3. **主动说取舍**：指出提高 Batch 有利吞吐却伤延迟，量化省带宽却可能受转换 Kernel 限制。
4. **最后落到项目**：固定负载分布做 Profile，监控 TTFT、TPOT、Goodput、HBM 带宽、FLOPs 和显存碎片。

**60 秒口述示例：**

> 我先给结论：定位推理瓶颈不能只看 GPU 利用率，要把 Prefill 和 Decode 分开，并判断是算力、内存带宽、显存容量还是调度受限。这里停一下，再用 Trace 拆出排队、Host 到 Device、Kernel 和通信时间，用 Roofline 看算术强度离硬件上限多远。取舍是增大 Batch 能提高吞吐，却会拉高 TTFT；压缩 KV 能省带宽，但转换可能抵消收益。项目里我会用真实长度与并发压测，报告 TTFT、TPOT、Goodput、HBM 带宽利用率和显存碎片率。
