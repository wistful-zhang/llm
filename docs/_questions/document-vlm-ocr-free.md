...
1. **先给结论：** 文档理解中 OCR-first 可解释且易检索，OCR-free 端到端保留版面视觉信息，选择取决于文本精度与布局复杂度。
2. **再讲关键机制：** 对比 OCR 检测识别再结构化的流水线，与图像编码器直接生成字段或文本的端到端路线。
3. **主动说取舍：** OCR-first 错误会级联但便于调试；OCR-free 简化链路，却需要更多数据且长文分辨率成本高。
4. **最后落到项目：** 按文档类型报告字符准确率、字段 F1、版面问答准确率、时延和人工复核率；说完停。

**60 秒口述示例：**

> 我会先给选择原则：票据和规整文本、需要可追溯字符时优先 OCR-first；复杂版面或文字与视觉强耦合时考虑 OCR-free。前者模块清晰但检测识别错误会级联，后者端到端却更吃数据和分辨率。项目里我会按扫描件、表格、手写等分桶，比较字符准确率、字段 F1、文档问答准确率、P95 时延和人工复核率。
