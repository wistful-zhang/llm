...
1. **先给结论：** 语音大模型通常把音频编码成语义或声学离散 Token，再由语言模型建模，解码器还原波形。
2. **再讲关键机制：** 区分语义 Token 与声学 Codec Token，并讲编码器、量化器、LLM 和声码器的串联。
3. **主动说取舍：** 更高码率保真度高却拉长序列；更强压缩省算力，但可能丢韵律、音色和细节。
4. **最后落到项目：** 在语音助手中报告 WER、说话人相似度、MOS、首包时延和实时率；到此停顿。

**60 秒口述示例：**

> 我会先把链路说清：音频经过编码器和量化器变成离散 Token，语义 Token 更关注内容，Codec Token 还承载音色与韵律；LLM 预测这些序列后，解码器恢复语音。核心取舍是码率与序列长度、保真度之间的平衡。项目里我会同时看 WER、说话人相似度、MOS、首个音频包时延和实时率，不能只听几个样例。
