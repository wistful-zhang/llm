...
1. **先给结论**：指出量化用精度换容量，Offload 用传输延迟换容量；说完停顿，先定瓶颈再选。
2. **再讲关键机制**：说明 KV 随层数、序列和并发增长，以及低比特存储、反量化、分层预取的工作方式。
3. **主动说取舍**：比较质量损失、Kernel 支持、PCIe 带宽与 Decode 关键路径风险。
4. **最后落到项目**：按热度保留 GPU 活跃窗口，用真实长度分布测质量、TPOT、吞吐、显存和总线流量。

**60 秒口述示例：**

> 我的结论是，KV 量化和 Offload 都在解决显存容量，但代价不同：量化付出精度与转换开销，Offload 付出跨总线搬运延迟。这里停一下，再解释 Decode 每步都要访问历史 KV，所以预取失败会直接拉高 TPOT，而低比特也只有被 Kernel 原生消费时才可能加速。项目里我会先测容量还是延迟受限，再按会话热度分层，GPU 保留活跃窗口，并用真实并发与上下文分布比较任务质量、P95 TPOT、吞吐、显存和 PCIe 流量。
