...
1. **先给结论：** 分布式数据加载要保证每个 rank 覆盖互斥且总体完整，并让断点同时恢复 epoch、shuffle 种子和样本游标。
2. **再讲关键机制：** 说明全局顺序如何由固定种子产生，再按 rank/worker 分片以及尾部样本处理。
3. **主动说取舍：** 严格无重复可能牺牲动态扩缩容灵活性；流式数据的精确恢复还需持久化源端偏移。
4. **最后落到项目：** 核验样本 ID 覆盖，报告重复率、遗漏率、data stall、GPU 空闲和恢复偏差；说完停。

**60 秒口述示例：**

> 我会先确定一个可复现的全局样本顺序，再按 rank 和 dataloader worker 做互斥分片；每个 epoch 更新同一随机种子。Checkpoint 不只记 epoch，还要记 shard、样本偏移和 shuffle 状态。扩缩容时需重新映射剩余样本。项目里我会汇总样本 ID，检查重复率和遗漏率都为零，并监控 data stall、GPU 空闲比例及恢复后的顺序偏差。 最后再说明扩缩容后的剩余样本映射。
