...
1. **先给结论：** 长任务系统要用持久化状态机和队列解耦请求，做到可取消、可恢复、幂等执行和进度可见。
2. **再讲关键机制：** 按提交、排队、租约领取、检查点、心跳、终态写入说明完整生命周期。
3. **主动说取舍：** 异步化提高可靠性和削峰能力，却引入重复投递、乱序事件和版本漂移。
4. **最后落到项目：** 以批量报告任务量化排队 P95、恢复成功率、重复副作用、取消生效时延和完成率；随后停。

**60 秒口述示例：**

> 我会先把同步请求改成任务资源：提交后立即返回 job id，worker 用租约消费，阶段性写检查点和心跳，取消只是状态转换，恢复时从最近一致检查点继续。队列通常是至少一次投递，所以副作用必须幂等。项目里我会监控排队 P95、卡死任务数、断点恢复成功率、重复副作用、取消生效时延和最终完成率。
...
1. **为什么队列消费成功不等于业务只执行一次？** 至少一次投递下，确认丢失或租约超时会让同一消息再次消费；业务副作用需要幂等键和唯一约束。
2. **任务取消与 worker 完成同时发生时，状态怎样收敛？** 用带版本的条件更新定义终态优先级，只允许一次状态转换成功，并让迟到结果按规则丢弃或保存为非正式产物。
3. **恢复长任务时为什么必须固定模型和 Prompt 版本？** 中途换版本会使前后阶段语义不一致，破坏可复现和审计；若必须升级，应创建新的任务版本并显式迁移。
