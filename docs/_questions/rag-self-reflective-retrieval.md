...
1. **先给结论**：把核心概括为“动态决定是否检索，并对证据和回答做显式评价”。
2. **再讲关键机制**：区分 Self-RAG 的训练式反思 Token 与 CRAG 的外部检索质量评估和纠错分支。
3. **主动说取舍**：多轮反思增加延迟，评估器本身也会误判，循环必须有预算和终止条件。
4. **最后落到项目**：以检索触发准确率、纠错增益、循环率、答案质量和 P95 做门控消融，之后停顿。

**60 秒口述示例：**

> 我会先说它们解决的是固定检索的局限：简单问题不必检索，难题则要判断证据够不够。Self-RAG 训练模型预测检索和批判类特殊 Token，CRAG 更像在系统外放一个检索评估器，低质量时触发改写或补充搜索。这里要强调评估器也会错，而且循环会增加尾延迟。项目里我会限制最大轮次，并比较检索触发准确率、纠错前后质量、无效循环率、平均调用次数和 P95。
