...
1. **先给结论**：用“知识放外部、行为写参数”给选择原则，并指出两者经常组合。
2. **再讲关键机制**：RAG 在推理时取证，微调通过梯度改变输出模式、领域语言和任务行为。
3. **主动说取舍**：RAG 有检索延迟和链路复杂度，微调更新慢、难引用且可能遗忘，均非万能。
4. **最后落到项目**：以知识新鲜度、可追溯性、任务正确率、迭代周期和总成本作决策表，答完停顿。

**60 秒口述示例：**

> 我会先给一个容易记的原则：频繁变化、需要引用和权限控制的事实知识优先 RAG；稳定的输出风格、格式和领域任务行为更适合微调。RAG 不修改参数，更新快但有检索延迟；微调推理链路短，却难保证把具体事实可靠写入模型，也可能遗忘。实际常用微调教会模型使用证据，再用 RAG 提供知识。项目中我会比较新鲜度、可追溯性、准确率、上线周期和总体成本后选方案。
