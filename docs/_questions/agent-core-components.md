---
title: "一个 LLM Agent 通常由哪些核心组件组成？"
source: "Datawhale 公开真实面试题整理；答案依据原论文和官方工程文档原创整理"
review_status: "待复习"
category: "Agent"
difficulty: "简单"
tags:
  - Agent
  - 执行循环
  - 状态
published: true
verified: true
date: 2026-07-13
---

## 面试时怎么答

建议按“结论 → 原理 → 取舍 → 落地”回答：

1. **先给结论：** 一个可用 Agent 至少包含模型、工具、状态/记忆、控制循环和安全护栏，生产系统还要有观测与退出条件。
2. **再讲关键机制：** 沿一次任务讲清模型决策、工具执行、观察回填、状态更新，直到完成或转人工。
3. **主动说取舍：** 自治程度越高适应性越强，但延迟、成本和不可预测性也越高，关键规则应交给确定性代码。
4. **最后落到项目：** 落到客服 Agent，量化任务完成率、工具正确率、平均步数、成本和升级人工率；随后停顿。

**60 秒口述示例：**

> 我会按一次闭环来答：模型负责理解和决策，工具连接外部能力，状态与记忆保存进度，控制器驱动观察、行动、再观察，护栏负责权限和停止条件。真正上线还需要追踪与回放。取舍是自治越强越灵活，也越难预测。项目中我会看任务完成率、工具参数正确率、平均步数、单任务成本和人工升级率。

## 核心回答

一个可执行任务的 Agent 通常包含目标与指令、负责决策的模型、可调用的工具、任务状态或上下文，以及控制循环。控制器把当前目标和可观察状态交给模型，模型提出下一步动作，系统校验并执行动作，再把结果作为 Observation 更新状态，直到达到完成条件、失败条件或步骤预算。Agent 是模型与运行时共同组成的系统，不只是一次 LLM 调用。

## 展开说明

常见组件可以这样理解：

1. **目标与策略约束**：定义任务、业务规则、允许和禁止的行为。
2. **模型或策略**：根据当前状态选择回答、提问、调用工具或结束。
3. **工具与环境**：把检索、计算、数据库和外部 API 作为受限动作暴露。
4. **状态与上下文**：保存用户输入、计划、工具结果、任务进度和必要记忆。
5. **编排器**：负责循环、路由、超时、重试、并发和终止条件。
6. **校验与观测**：检查参数、权限、结果和真实终态，并记录可回放轨迹。

不同框架的命名并不统一，组件也不一定都由模型驱动。例如权限校验和高风险确认应由确定性代码控制。

## 工程实践

先画清状态机和信任边界：哪些字段来自用户，哪些由服务端生成，哪些动作有副作用。为每次运行分配 run_id，保存状态转换、工具调用和最终结果；同时设置最大步骤、最大费用和取消机制，避免 Agent 无界运行。

## 常见追问

1. **Agent 与一次带工具的模型调用有什么区别？** 单次调用只产生一次决策；Agent 有持续的状态和观察—行动循环，能根据工具结果重规划，直到完成、失败或转人工。
2. **哪些组件必须由确定性代码控制？** 身份认证、权限、金额和配额校验、幂等、状态迁移及危险动作审批不能只依赖模型判断。
3. **Agent 在什么条件下应该停止或向用户追问？** 达到可验证终态应停止；缺少会实质改变结果的信息、连续失败、风险超阈值或预算耗尽时，应追问或升级人工。

## 一句话复习

> Agent 是“模型决策、工具执行、状态更新和受控循环”的组合系统。

## 参考资料

- 面经主题：[Datawhale 真实面试题中的 Agent 核心组件](https://github.com/datawhalechina/hello-agents/blob/main/Extra-Chapter/Extra01-%E9%9D%A2%E8%AF%95%E9%97%AE%E9%A2%98%E6%80%BB%E7%BB%93.md)
- 技术依据：[ReAct](https://arxiv.org/abs/2210.03629)、[Anthropic：Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents)
