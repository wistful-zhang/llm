...
1. **先给结论**：先说请求数不能代表 LLM 负载，应组合请求速率、Token 预算、并发和优先级；停顿后讲两阶段计费。
2. **再讲关键机制**：解释请求前估算并预留输入加最大输出预算，流式完成或取消后按实际用量结算回补。
3. **主动说取舍**：指出按 max_tokens 全额预留最安全却降低利用率，乐观估算提高吞吐但可能超卖。
4. **最后落到项目**：本地桶抗突发、全局账本守租户配额，监控拒绝、超用、估算误差、公平性和 429 重试。

**60 秒口述示例：**

> 我的结论是，LLM 网关不能只按 QPS 限流，因为一次请求可能消耗几十或几万 Token。我会同时限制请求、Token、并发和费用。这里停一下，再讲请求前按输入和输出上限预留额度，流式结束、取消或超时后按实际用量结算回补。项目里用本地桶挡突发、全局配额保证租户公平，并监控 429 率、预算超用、估算误差、排队 P95 和每租户 Goodput。
