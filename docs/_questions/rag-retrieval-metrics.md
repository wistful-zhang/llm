...
1. **先给结论**：用“找没找到、第一条多靠前、整列排序多好”分别对应 Recall@K、MRR、nDCG。
2. **再讲关键机制**：写出分母和折损直觉，说明二元相关与分级相关标签的差别。
3. **主动说取舍**：单一指标会遮蔽问题；高 Recall 不代表上下文可用，也不代表最终回答正确。
4. **最后落到项目**：按问题类型报告 Recall@K、MRR、nDCG 与端到端正确率，并给 Bootstrap 置信区间。

**60 秒口述示例：**

> 我会用三个问题来答：Recall@K 看 K 条里覆盖了多少相关证据；MRR 看第一条相关证据排得多靠前；nDCG 同时考虑多个结果的相关等级和位置折损。若每题只有一个证据，MRR 很直观；若有多条且重要程度不同，nDCG 更合适。这里要强调检索分数不是最终质量。项目里我会按查询类型分桶报告三项指标、Bootstrap 置信区间，再与答案正确率和 Faithfulness 做误差交叉分析。
