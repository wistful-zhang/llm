...
1. **先给结论**：先说 Transformer 训练时可并行处理同层所有位置，RNN 沿时间步存在递归依赖。
2. **再讲关键机制**：对比 Attention 的全序列矩阵乘与 RNN 的 `h_t=f(h_(t-1),x_t)`，再补路径长度。
3. **主动说取舍**：标准 Attention 有二次长度成本，自回归 Decode 仍逐 Token；RNN 流式有界状态仍有优势。
4. **最后落到项目**：按训练 Tokens/s、长序列显存、Decode 延迟和任务质量比较，而不只讲理论复杂度。

**60 秒口述示例：**

> 我的结论是 Transformer 更适合大规模训练，关键不是完全没有串行，而是同一层所有序列位置可以通过矩阵乘并行；RNN 的第 t 个隐藏状态依赖第 t-1 个，时间维难并行。Self-Attention 还缩短远距离信息路径，但标准实现有 `n²` 连接成本，且自回归 Decode 仍要逐 Token。项目选型时我会看训练 Tokens/s、长上下文显存、首 Token 与逐 Token 延迟、流式状态约束和最终任务质量。
...
但 Transformer 并非“完全并行”：不同层仍按前后顺序执行，集合通信也可能同步等待；自回归推理时下一个 token 依赖前一个 token，生成步之间仍不能并行。标准稠密 Attention 对长度 `n` 会形成 `n × n` 的分数矩阵，长序列的计算、显存与 IO 因而成为重要瓶颈。

## 展开说明

训练阶段的差异可以从三点比较：

- **顺序关键路径**：经典 RNN 约有 `O(n)` 个连续时间步；Transformer 单层内可同时处理所有已知位置，但层数方向仍是顺序的。
...
若隐藏维度为 `d`、序列长度为 `n`，标准 Self-Attention 的分数计算约为 `O(n²d)`，分数矩阵空间约为 `O(n²)`；FFN 通常约为 `O(nd²)`。因此在 `n` 很大时 Attention 可能占主导，在 `d` 很大、序列较短时 FFN 也可能是主要计算。FlashAttention 通过分块和重计算减少 HBM IO 与中间显存，并没有把精确稠密 Attention 的理论二次计算量普遍改成线性。
...
3. **Self-Attention 的时间和空间复杂度分别是什么？** 标准全注意力的打分计算约为 `O(n²d)`，显式分数矩阵空间约为 `O(n²)`；完整 Block 还包含约为 `O(nd²)` 的投影与 FFN。
