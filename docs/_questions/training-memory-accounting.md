...
1. **先给结论**：先把显存拆成参数、梯度、优化器、激活、临时工作区和碎片六项。
2. **再讲关键机制**：按参数量与字节数手算静态项，再说明激活随 Batch、序列和层数增长。
3. **主动说取舍**：Checkpointing 只省激活并增加重算，不省优化器；分片省常驻状态却增加通信。
4. **最后落到项目**：用峰值 allocated/reserved、Tokens/s、重算比例和通信时间校准理论估算后停顿。

**60 秒口述示例：**

> 我会先给显存账本：权重、梯度、Adam 一二阶状态、可能的 FP32 主权重、前向激活、临时 Buffer 和碎片。静态项可按参数量乘每元素字节估算，激活则与 micro-batch、序列长度、隐藏维度和层数有关。Gradient Checkpointing 用反向重算换激活显存，不会减少优化器状态。项目中我会拿框架的 allocated、reserved 和峰值校准公式，同时看重算后的 Tokens/s、通信占比和碎片，保留 OOM 安全余量。
