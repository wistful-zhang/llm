...
1. **先给结论：** Beam Search 累加对数概率天然偏向短序列，因为每生成一个 Token 通常都会再加一个负数。
2. **再讲关键机制：** 写出累计 log-prob 分数，再说明长度归一化、length penalty 和 EOS 分数如何改变排序。
3. **主动说取舍：** 惩罚过弱会过早结束，过强会生成冗长文本，最优系数依任务和最大长度而变。
4. **最后落到项目：** 在摘要或翻译上同时测任务质量、输出长度分布、过早 EOS 率和延迟；说完停。

**60 秒口述示例：**

> 我会先说原因：Beam Search 比较的是累计 log 概率，每一步的 log 概率通常为负，所以长候选会持续被扣分，EOS 早出现的短序列容易胜出。常用长度归一化或 length penalty 调整，但它会引入反向偏差。项目里我会在固定 beam 下扫描系数，同时看 ROUGE 或 BLEU、长度分布、过早 EOS 比例、重复率和 P95 解码时延。 若继续追问，我再解释 EOS 的打分边界。
