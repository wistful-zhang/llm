...
1. **先给结论**：先说 DP 复制计算分数据，TP 切层内张量，PP 切连续层，3D 并行是三维组合。
2. **再讲关键机制**：分别指出 DP 梯度归约、TP 层内集合通信和 PP 激活传递与 Micro-batch 调度。
3. **主动说取舍**：TP 通信频繁需快互联，PP 有气泡与阶段不均，DP 受全局 Batch 和完整副本限制。
4. **最后落到项目**：先按单层显存确定 TP，再按节点切 PP，剩余卡给 DP；用 MFU、气泡率、通信占比验收。

**60 秒口述示例：**

> 我的结论是三种并行切不同维度：数据并行让每组处理不同样本再归约梯度；张量并行把单层矩阵切到多卡；流水线并行把连续层分 Stage，用 Micro-batch 填充。设计时先确保单层权重和激活能放下，通常把 TP 放在高速互联域，再按节点切 PP，剩余规模用于 DP。取舍是 TP 通信频繁、PP 有气泡、DP 放大全局 Batch。项目中看 MFU、通信占比、气泡率、显存余量和弱扩展效率。
