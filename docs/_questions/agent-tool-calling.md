...
1. **先给结论：** Tool Calling 是模型生成受 Schema 约束的调用意图，由应用校验并执行，再把结果返回模型继续推理。
2. **再讲关键机制：** 依次说明工具描述、名称选择、参数生成、JSON 校验、权限校验、执行和结果回填。
3. **主动说取舍：** Schema 只能约束结构，不能证明参数在业务上安全，也不能保证模型选对工具。
4. **最后落到项目：** 用订单查询案例衡量工具选择准确率、参数一次通过率、执行成功率和误调用率；到此停顿。

**60 秒口述示例：**

> 我会先讲完整链路：应用把工具名称、说明和 Schema 给模型，模型输出调用及参数；应用先做结构、权限和业务校验，执行后再把结果回填，模型才生成最终答案。这里 Schema 只保证格式，不是安全边界。项目里我会拆开统计工具选择准确率、参数一次校验通过率、真实执行成功率、误调用率和重试成本。
