...
1. **先给结论**：先说流式链路必须有有界缓冲、可传播取消和明确慢消费者策略。
2. **再讲关键机制**：沿推理调度器、应用层队列、SSE/WebSocket、客户端消费讲背压如何传递。
3. **主动说取舍**：暂停可保输出但占 GPU 序列槽，取消省资源却丢结果，无界缓存一定不可接受。
4. **最后落到项目**：报告首 Token、Token 间隔、缓冲深度、慢客户端率、取消释放时间和内存上限。

**60 秒口述示例：**

> 我的结论是 Token Streaming 不能靠无限缓存掩盖慢客户端。每个请求要有有界队列，连接断开或消费超时后把取消信号从网关传到推理调度器，在安全迭代边界移除序列并释放 KV Cache。只有服务端推送文本通常用 SSE，需要真正双向通信再用 WebSocket。取舍是暂停会占用 GPU 槽位，取消则丢结果。项目中我会监控 TTFT、Token 间隔、队列深度、慢消费者比例、取消释放时长和进程内存。
