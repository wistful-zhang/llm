...
1. **先给结论**：先说生成概率不等于答案正确概率，置信度必须针对任务校准；结论后停顿。
2. **再讲关键机制**：解释可靠性图、ECE、Brier Score 与选择性风险，并说明需要可验证标签。
3. **主动说取舍**：指出提高拒答率可降错误风险，却会牺牲覆盖率并可能造成群体差异。
4. **最后落到项目**：训练轻量校准器并按风险设阈值，监控覆盖率、选择性准确率、ECE 和人工转接率。

**60 秒口述示例：**

> 我会先说结论：模型 Token 概率高，不代表整条回答正确，所以置信度要在具体任务和分布上重新校准。这里停一下，再讲我会用有标签验证集画可靠性图，计算 ECE 或 Brier Score，并考察拒答后剩余样本的选择性风险。取舍是阈值越严格，错误率可能越低，但覆盖率和用户体验会下降。项目里我会把模型特征、检索证据和规则信号输入校准器，按业务损失定阈值，持续监控覆盖率、选择性准确率、ECE 与人工转接率。
