...
1. **先给结论**：先说离线推理核心是可分片、幂等、可恢复、可审计，不是简单循环调用 API；停顿后讲状态机。
2. **再讲关键机制**：解释输入快照、确定性任务 ID、分区、检查点、重试队列和原子结果提交。
3. **主动说取舍**：指出小分片恢复精细却调度开销大，激进重试会重复计费或压垮依赖。
4. **最后落到项目**：按长度分桶提高批处理，监控完成率、重复率、重试率、GPU 利用率、吞吐和单位成本。

**60 秒口述示例：**

> 我先给结论：可靠的离线批量推理要把每条任务做成可分片、幂等、可恢复、可审计的状态机，不能靠进程从头跑到尾。这里停一下，再讲输入先冻结快照，用内容和版本生成任务 ID，Worker 按长度分桶批处理，结果先写临时区再原子提交，失败按可恢复类型进入有限重试。取舍是分片越小恢复越精细，但调度和元数据成本越高。项目里我监控完成率、重复提交率、重试率、GPU 有效利用率、每秒 Token 和每百万 Token 成本。
