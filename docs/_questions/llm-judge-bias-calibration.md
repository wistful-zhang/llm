...
1. **先给结论**：先说 LLM Judge 可扩展但不是客观真值，必须校准和抽检；一句话后停顿。
2. **再讲关键机制**：解释位置、长度、风格与自增强偏差，并说明交换顺序和成对比较。
3. **主动说取舍**：指出多次采样提高稳定性却增加成本，复杂 Rubric 提高覆盖却降低一致性。
4. **最后落到项目**：用人工金标校准 Judge，监控与人一致率、位置翻转率、平局率和评测成本。

**60 秒口述示例：**

> 我先给结论：LLM-as-a-Judge 适合扩大评测规模，但不能当成绝对真值。这里停一下，再讲常见偏差：答案放前后的位置、长度、格式和 Judge 对同源模型的偏好都会改变结论。我会交换候选顺序、隐藏模型身份、用明确 Rubric，并对不稳定样本多次评审。取舍是校准越充分成本越高，因此高风险场景保留人工金标抽检。项目指标包括 Judge 与人工一致率、交换顺序后的翻转率、平局率、各切片偏差和每千样本评测成本。
