...
1. **先给结论**：先给排查顺序“数据与标签 → 小样本过拟合 → 数值与优化器 → 分布式与评测”。
2. **再讲关键机制**：检查 Chat Template、Label Shift、`-100` 比例、学习率、梯度和非有限值。
3. **主动说取舍**：大规模盲调超参数成本高且掩盖根因；Loss 下降也可能只是学会格式或重复答案。
4. **最后落到项目**：以单 Batch 过拟合、有效标签率、梯度范数、Loss 曲线和任务回归逐层设门禁。

**60 秒口述示例：**

> 我不会先调学习率，而是先验证数据链路：解码一批 Token，检查 Chat Template、角色边界、Label Shift 和有效标签比例。然后拿几十条高质量样本关闭复杂并行做过拟合；如果仍不降，查 Loss、优化器、梯度和混合精度，能过拟合再扩大数据与分布式。这里要提醒 Loss 下降不代表成功。项目中我会记录有效 Token 比、梯度范数、非有限值、训练验证差距和真实任务成功率，每一步只改一个变量。
