...
1. **先给结论**：先说多图/视频比单图多了跨帧对应、时序、Token 爆炸和输入边界四类问题；列完停顿。
2. **再讲关键机制**：解释采帧、视觉编码、时间/图像位置标识、压缩与跨模态注意力。
3. **主动说取舍**：指出采帧密集保留动作却增加延迟，强压缩省 Token 又可能漏掉短事件与小目标。
4. **最后落到项目**：按任务自适应采样并保留时间戳，监控事件召回、时序准确率、视觉 Token、延迟和显存。

**60 秒口述示例：**

> 我的结论是，多图和视频不是把多张图片简单拼接，它新增跨帧实体对应、事件时序、视觉 Token 爆炸和图像边界混淆。这里停一下，再讲典型流程：按镜头或任务采帧，视觉编码后加入帧号与时间信息，再做 Token 压缩并与文本交互。取舍是高帧率能保留短动作，却显著增加显存和延迟；压缩过强又会漏小目标。项目里我会按事件自适应采样，报告关键事件召回率、时序问答准确率、平均视觉 Token、P95 延迟和峰值显存。
