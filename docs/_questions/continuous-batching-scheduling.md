...
1. **先给结论：** Continuous Batching 在每个生成迭代动态加入和移出请求，按 token 预算组批，比静态等待整批结束更高效。
2. **再讲关键机制：** 讲清 prefill/decode 队列、每步重组 batch、KV Cache 占用和抢占或换出策略。
3. **主动说取舍：** 吞吐提高会带来调度复杂度与公平问题，追求满载可能恶化单请求尾延迟。
4. **最后落到项目：** 给出吞吐、TTFT/TPOT P95、队列等待、抢占率和租户公平性；指标后停。

**60 秒口述示例：**

> 我会先对比静态批处理：静态 batch 要等最长请求结束，Continuous Batching 则每个 decode step 都能移除完成请求、加入新请求，持续填满 GPU。调度器还要受 token 预算和 KV Cache 限制，必要时抢占。取舍是吞吐与公平、尾延迟冲突。项目中我会同时看 tokens/s、TTFT 和 TPOT 的 P95、排队时延、抢占率及各租户 SLO。 如果追问公平性，我再讲抢占和请求老化。
