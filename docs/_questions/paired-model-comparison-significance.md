...
1. **先给结论**：先说同题成对比较差值，并报告效应量与置信区间，不能只比两个平均分。
2. **再讲关键机制**：说明按题重采样的 Paired Bootstrap，或针对二元胜负使用 McNemar 检验。
3. **主动说取舍**：显著不等于业务重要，多次切片检验还会提高假阳性，Judge 偏差也可能主导差异。
4. **最后落到项目**：预先定义最小有意义提升，报告胜负平、区间、功效和关键切片后停顿。

**60 秒口述示例：**

> 我会让两个模型回答完全相同的问题，再对每题的分数或盲评胜负做成对分析，这能消除题目难度造成的大量方差。连续指标可按题做 Paired Bootstrap 得到差值置信区间，二元正确与否可用 McNemar；同时报告效应量，不只报 p 值。这里要提醒显著不等于值得上线，还要预先定义最小业务提升，并对多切片检验做校正，检查 Judge 的位置和长度偏差。
