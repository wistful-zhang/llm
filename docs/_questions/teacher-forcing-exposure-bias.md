...
先说训练和推理看到的历史不同：Teacher Forcing 训练时把真实前一 Token 喂给模型，便于并行和稳定优化；推理时只能吃自己的输出，一次错误会改变后续输入分布，这就是 Exposure Bias。

追问缓解方法时，可谈更贴近生成轨迹的数据、序列级目标、Scheduled Sampling 或偏好优化，同时指出 Scheduled Sampling 也可能带来训练目标不一致，并非万能解法。

**可以这样答：**

> Teacher Forcing 在训练第 t 个 Token 时使用真实前缀，而不是模型刚才预测的 Token，因此交叉熵训练稳定且能并行。自回归推理只能基于自己的历史，一旦前面写错，后面看到的上下文就偏离训练分布，错误可能继续累积，这就是 Exposure Bias。混入模型生成前缀或做序列级训练可以缓解，但也会让训练目标更难优化，所以并不存在没有代价的修复办法。
...
这里的 $$x_{<t}$$ 来自真实样本。推理时的条件前缀则包含模型样本 $$\hat{x}_{<t}$$：
