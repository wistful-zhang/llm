...
1. **先给结论**：先说 Teacher Forcing 用真实前缀训练，Exposure Bias 来自推理时改用模型自己的前缀。
2. **再讲关键机制**：解释真实目标序列已知使所有位置可并行计算，以及错误前缀如何造成分布偏移。
3. **主动说取舍**：Teacher Forcing 训练稳定高效；混入模型采样能缩小差异，却破坏并行并引入新偏差。
4. **最后落到项目**：同时看 Token Loss、自由生成成功率、长输出错误累积率与多轮恢复能力，答完停顿。

**60 秒口述示例：**

> 我的结论是 Teacher Forcing 在训练第 t 个 Token 时喂真实的前缀，因此目标序列一次已知，可在 Causal Mask 下并行算所有位置；推理时前缀来自模型自己，一旦前面出错，后续条件就偏离训练分布，这叫 Exposure Bias。它不代表错误必然无限放大，模型也可能恢复。项目中我不会只看 Teacher-forced Loss，还会做自由生成、多轮和长输出评测，统计首错位置、错误累积率和恢复率，再决定是否需要数据或解码治理。
