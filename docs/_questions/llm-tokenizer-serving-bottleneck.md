...
Tokenizer 瓶颈要沿模板渲染、UTF-8 预处理、编码、入队、增量解码和流式发送逐段测时，证明 GPU 是否真的在等 CPU。优化可谈 Fast Tokenizer、批量化、受限线程池和版本绑定缓存；单纯加线程可能增加切换、内存与排队。

**可以这样答：**

> 长 Prompt 或大量短请求下，GPU 可能在等 CPU 完成模板渲染、分词和流式解码。我会拆分 Encode、排队、GPU 与 Decode 耗时，确认而不是猜。优化上用 Fast Tokenizer、批量调用和受限线程池，对不可变公共前缀按 Tokenizer 与模板版本缓存。定位问题时监控分词队列、P95 编码耗时、每秒 Token、CPU 饱和度和 TTFT。
