...
1. **先给结论：** 成对偏好可用 Bradley–Terry 把模型潜在能力映射为胜率，Elo 是其在线更新式实现，Arena 还需处理位置和用户偏差。
2. **再讲关键机制：** 写出 `P(i胜j)=σ(r_i-r_j)`，说明由比较数据估计 rating、置信区间和 bootstrap。
3. **主动说取舍：** 排名直观但不是绝对质量；对手分布、提示分布、位置偏差和非平稳模型都会影响结果。
4. **最后落到项目：** 报告胜率、rating 置信区间、位置偏差、有效票数和关键类别分桶；说完停。

**60 秒口述示例：**

> 我会先用 Bradley–Terry 解释：假设每个模型有潜在分数，i 战胜 j 的概率是两者分差经过 sigmoid。Elo 则收到一场结果后在线更新分数。它适合聚合成对偏好，但 rating 不是绝对能力，还依赖对手和提示分布。项目里我会随机左右位置、过滤异常投票，并报告胜率、rating 的 bootstrap 置信区间、位置偏差和分领域结果。
