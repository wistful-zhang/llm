...
1. **先给结论：** Benchmark 去污染要在训练前做多粒度匹配，并把检测阈值、时间戳和移除策略完整记录。
2. **再讲关键机制：** 从精确哈希、规范化文本、n-gram/MinHash 到语义近邻说明由严到宽的检测链路。
3. **主动说取舍：** 阈值宽会误删同领域正常数据，阈值窄又漏掉改写、翻译和答案泄漏。
4. **最后落到项目：** 报告题目与答案分别的命中率、人工抽检精度、移除 token 数和污染前后分数；讲完停。

**60 秒口述示例：**

> 我会先说明去污染不只是查完全重复。先对题目、答案做规范化精确匹配，再用 n-gram 或 MinHash 找局部重合，必要时用语义检索捕捉改写，并按数据时间戳审计。阈值过宽会误删正常训练样本。项目里我会分别报告题目命中和答案命中、人工抽检精度、移除 token 比例，以及清洗前后的基准分数差。
