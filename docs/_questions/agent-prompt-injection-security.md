...
1. **先给结论：** 防 Prompt Injection 不能靠一句系统提示，必须把外部内容当数据并用权限边界限制其可造成的动作。
2. **再讲关键机制：** 按不可信内容标记、指令来源优先级、工具最小权限、参数校验和高风险审批说明纵深防御。
3. **主动说取舍：** 过滤器能拦常见模式但有漏报，过严策略又会损害正常工具能力。
4. **最后落到项目：** 用邮件 Agent 红队集验证攻击成功率、误拦率、越权调用数和人工审批率；讲完停住。

**60 秒口述示例：**

> 我会先说结论：提示注入本质上不能只在模型层解决。邮件和网页都按不可信数据处理，模型提出动作后还要经过工具白名单、最小权限、参数策略和高风险人工审批，敏感数据也要按用途隔离。取舍是策略越严，正常任务可能受损。项目中我会用红队样本测攻击成功率、正常请求误拦率、越权调用数和审批负担。
