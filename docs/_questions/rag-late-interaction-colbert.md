...
1. **先给结论**：先把 ColBERT 放在单向量双塔和 Cross-Encoder 之间，点出效果与效率的折中。
2. **再讲关键机制**：解释查询和文档独立编码成 Token 向量，以及每个查询 Token 的 MaxSim 聚合。
3. **主动说取舍**：细粒度匹配提升效果，但多向量索引带来显存、存储和内存访问压力。
4. **最后落到项目**：比较 Recall/nDCG、索引每文档字节、查询 P95 和重排成本，停下来等 MaxSim 追问。

**60 秒口述示例：**

> 我的结论是 ColBERT 是一种 Late Interaction：查询和文档先各自编码，所以文档仍可离线建索引；但它不压成单个向量，而是保留 Token 向量，查询时每个查询 Token 找文档中的最大相似度再求和。因此表达力比普通双塔强，成本又低于全量 Cross-Encoder。主要代价是多向量索引更大。项目里我会同时比较 nDCG、端到端答案质量、每文档索引字节、P95 延迟和 GPU 内存访问。
