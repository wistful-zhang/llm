...
先把 ColBERT 放在双塔与 Cross-Encoder 之间：文档仍可离线编码，但不急着池化成单向量；查询 Token 与文档 Token 在检索时做 MaxSim 聚合，因此保留了更细的匹配信号。

追问一般集中在成本。说明 Token 级向量带来更大的索引和更多在线计算，需要压缩、剪枝和候选检索；它并没有达到 Cross-Encoder 的完整跨文本注意力。

**可以这样答：**

> ColBERT 分别编码查询和文档，所以文档表示可以预计算；与普通双塔不同，它保留每个 Token 的向量。检索时，每个查询 Token 找文档 Token 中的最大相似度，再把这些 MaxSim 分数相加，既能捕捉细粒度词项对应，又避免对每个候选做联合编码。代价是索引体积和在线打分更大，通常要配合量化、剪枝和两阶段检索。
