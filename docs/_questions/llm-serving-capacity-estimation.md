...
1. **先给结论**：先说容量由请求长度、到达率、SLO、模型与硬件共同决定，不能只看 QPS；停顿后算资源。
2. **再讲关键机制**：分别估算权重、KV 与运行时显存，再用压测得到单副本可持续 Goodput。
3. **主动说取舍**：说明高利用率节省成本却缺少突发余量，多副本冗余则增加闲置。
4. **最后落到项目**：按峰值切片和故障场景做 N+1 规划，监控 Goodput、排队、P99、拒绝率和余量。

**60 秒口述示例：**

> 我的结论是，LLM 容量不能从 QPS 直接换算 GPU 数，因为同样 QPS 的输入输出长度和延迟目标可能完全不同。这里停一下，再分两步：先算权重、KV Cache 和运行时缓冲能否放下目标并发，再用真实长度分布压测单副本在 SLO 内的 Goodput。取舍是把 GPU 吃满成本低，却没有流量突发和故障余量。项目里我按高峰、租户和地域切片做 N+1 规划，并持续看排队时间、P99 TTFT/TPOT、拒绝率、显存水位和可用余量。
