...
正交旋转保持向量范数，并利用 `R_mᵀR_n = R_(n-m)` 把绝对位置差转进 Q/K 点积。RoPE 并不意味着模型只知道相对位置：因果边界、内容表示和训练分布仍会共同影响最终行为。
...
2. **如何从旋转矩阵推导内积只与相对位置有关？** 利用旋转矩阵正交性可得 `R_mᵀR_n = R_(n-m)`，所以 `(R_mq)ᵀ(R_nk) = qᵀR_(n-m)k`。位置项由两个绝对位置变成了它们的差。
