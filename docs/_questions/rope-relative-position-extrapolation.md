...
1. **先给结论**：先说 RoPE 通过旋转 Q/K 让注意力内积显式依赖相对位移。
2. **再讲关键机制**：用二维旋转块和 `R_m^T R_n=R_(n-m)` 解释相对位置，再讲高低频。
3. **主动说取舍**：超出训练长度会遇到相位分布外问题，线性插值扩范围却压缩局部位置分辨率。
4. **最后落到项目**：用不同长度下困惑度、检索准确率、位置扰动和 Decode 延迟验证，停一下再谈缩放方案。

**60 秒口述示例：**

> 我会先给结论：RoPE 不是把位置向量直接相加，而是按位置旋转 Q 和 K。因为旋转矩阵满足 `R_m^T R_n=R_(n-m)`，注意力内积中的位置项只与相对距离有关。长上下文难点是模型遇到训练范围外的旋转相位；插值或频率缩放能扩范围，但可能损失局部分辨率。工程上我不会只改最大长度，而会按长度测困惑度、长文检索、位置鲁棒性、显存和 Decode 延迟。
