...
1. **先给结论**：用“BM25 保精确词，向量检索保语义召回”解释为何组合，并先停一下。
2. **再讲关键机制**：说明两路独立取候选，再用 RRF 或归一化分数融合，必要时接 Reranker。
3. **主动说取舍**：混合检索增加存储和查询成本，直接相加异质分数又会造成不可控偏置。
4. **最后落到项目**：按查询类型分桶调权重，比较 Recall@K、MRR、无结果率、P95 延迟和索引成本。

**60 秒口述示例：**

> 我的结论是 BM25 和稠密检索互补：前者擅长人名、编号和精确关键词，后者能找语义相近但用词不同的文本。工程上让两路分别召回，再用 RRF 按名次融合，因为 BM25 分数和余弦分数不能直接相加，最后可对小候选集重排。代价是两套索引和更高延迟。项目里我会按实体型、自然语言型查询分桶，比较 Recall@K、MRR、无结果率和 P95，再决定融合权重。
