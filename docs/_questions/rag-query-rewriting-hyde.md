...
1. **先给结论**：先说 Rewrite 修正查询表达，HyDE 则生成假想答案文档来跨越查询与语料的表述差距。
2. **再讲关键机制**：对比实体约束改写、多查询扩展与“生成假文档后取向量检索”的流程。
3. **主动说取舍**：两者都可能改坏意图；HyDE 还会引入错误实体和一次生成调用的延迟。
4. **最后落到项目**：按短查询、歧义查询、术语查询分桶比较 Recall、漂移率、P95 和调用成本后停顿。

**60 秒口述示例：**

> 我的回答会先区分目标：Query Rewrite 把口语、省略和歧义改成更可检索的表达；HyDE 先生成一段假想答案，再用它的向量去找真实文档，利用答案体和文档体更接近的语义空间。它们都不是无条件开启，改写可能丢否定、时间和实体，HyDE 也可能漂移。落地时我会保留原查询并融合召回，按查询类型比较 Recall@K、意图漂移率、P95 延迟和额外调用成本。
