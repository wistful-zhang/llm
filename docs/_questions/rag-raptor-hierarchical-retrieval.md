...
1. **先给结论**：先说 RAPTOR 用“叶子原文 + 多层聚类摘要”同时提供细节和全局语义；停顿后讲建树。
2. **再讲关键机制**：按切块、Embedding、聚类、摘要、递归建层和查询检索回答。
3. **主动说取舍**：指出它适合长文档概括与跨段推理，却增加索引成本、更新复杂度和摘要失真风险。
4. **最后落到项目**：保留摘要到原文血缘并与平铺基线对照，报告 Recall、答案正确率、引用率、构建成本和新鲜度。

**60 秒口述示例：**

> 我的结论是，RAPTOR 不只检索固定 Chunk，而是把叶子片段聚类、摘要，再递归形成多层树，让查询既能拿到原文细节，也能命中文档全局主题。这里停一下，再说它适合长文总结和跨段推理，但摘要可能失真，增量更新也更复杂。项目里我保留每个摘要到原文的血缘，并与平铺检索比较 Recall@K、答案正确率、引用可验证率和索引构建成本。
