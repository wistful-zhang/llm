...
1. **先给结论**：先说明 GraphRAG 的优势是回答跨文档全局问题，不是所有 RAG 的默认升级。
2. **再讲关键机制**：按实体关系抽取、图构建、社区检测、分层摘要、Map-Reduce 回答讲清链路。
3. **主动说取舍**：建图和摘要昂贵、更新复杂且会引入抽取与摘要误差，点查任务可能得不偿失。
4. **最后落到项目**：用全局问题覆盖率、引用回溯正确率、索引更新时间和单查询成本与普通 RAG 对照。

**60 秒口述示例：**

> 我会先界定适用场景：GraphRAG 主要解决答案分散在许多文档中的全局问题。它从文本抽取实体和关系建图，再做社区检测与分层摘要，查询时组合相关社区报告，而不是只取几个相似 Chunk。这里要主动说成本，抽取、摘要和增量维护都贵，还可能层层失真。落地时我会与混合 RAG 做对照，比较全局问题正确率、来源回溯率、索引构建时长、更新延迟和单次查询成本。
