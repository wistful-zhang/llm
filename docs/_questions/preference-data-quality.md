...
1. **先给结论**：先说好的偏好对必须同 Prompt、可比较、有明确差异且不靠长度或格式投机；停顿后讲采集。
2. **再讲关键机制**：解释候选生成、Rubric、盲化随机顺序、多标注者与分歧处理。
3. **主动说取舍**：指出差异太小信号弱，差异太大任务又过易；过滤分歧会提升一致性却损失真实多样性。
4. **最后落到项目**：做去重、长度配平和时间切分，监控一致率、平局率、偏差切片、胜率和数据泄漏率。

**60 秒口述示例：**

> 我的结论是，chosen/rejected 不是随便找一好一坏，而要在同一 Prompt 和同一评价标准下可比较，且不能让模型仅凭长度、格式或固定措辞猜标签。这里停一下，再讲候选应覆盖相近难度，标注时隐藏来源、随机顺序、给出 Rubric，并保留平局与标注者分歧。取舍是只留高度一致样本更干净，却会丢掉主观和长尾需求。项目里我会语义去重、长度配平、按用户与时间切分，报告标注一致率、平局率、长度偏差、盲测胜率和评测泄漏率。
