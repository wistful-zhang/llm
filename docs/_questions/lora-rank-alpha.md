...
增大 $$r$$ 给了更新更多方向，但也增加显存、计算和过拟合空间；增大 $$\alpha$$ 会放大同一组 $$A/B$$ 产生的更新。二者不是“越大越好”，也不存在脱离模型、数据和学习率的统一最优组合。
...
1. **Rank 增大后 Alpha 必须同比增大吗？** 不必须；同比增大可保持 $$\alpha/r$$，但最佳尺度仍要结合学习率和验证结果确定。
