...
1. **先给结论：** 多 Agent 适合可并行、角色专业且结果可合并的任务，不应把它当成默认架构。
2. **再讲关键机制：** 解释编排者拆分任务、Worker 交付结构化结果、聚合器解决冲突和共享状态版本。
3. **主动说取舍：** 它能提升并行度和覆盖面，却增加通信 token、重复劳动、冲突与级联失败。
4. **最后落到项目：** 以研究任务为例，对比单 Agent 的成功率、墙钟时延、token 成本和冲突率；到此停顿。

**60 秒口述示例：**

> 我会先给使用条件：任务能清楚拆分、子结果能独立验收并且并行收益大于通信成本时，才用多 Agent。编排者负责分派和预算，Worker 返回结构化证据，聚合器处理冲突与重复。它的风险是级联失败和成本膨胀。项目里我会与单 Agent 做基线，对比任务成功率、墙钟时延、总 token、重复工作率和结果冲突率。
