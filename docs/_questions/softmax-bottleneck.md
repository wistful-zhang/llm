...
从输出层的矩阵形式解释：隐藏状态经线性投影再 Softmax，跨上下文形成的对数概率矩阵，其秩受隐藏维度限制；真实语言分布可能需要更高秩，所以称为瓶颈。

被问解决方案时，可以举 Mixture of Softmaxes、非线性输出头或增大表示维度，但也要说实际收益会受模型主体、参数共享和计算成本影响，不能只凭秩断言效果。

**可以这样答：**

> 标准语言模型用一个 d 维隐藏状态经过线性词表头和 Softmax 得到分布。把许多上下文的对数概率排成矩阵后，它可表示的秩大致受隐藏维度加常数项限制，而真实条件分布可能具有更高秩，这就是 Softmax Bottleneck。Mixture of Softmaxes 通过混合多个条件分布突破单个线性头的秩限制，代价是参数和计算增加；瓶颈说的是输出族的表达上限，不等于每个模型都已被它卡住。

## 核心回答

Softmax Bottleneck 指线性 LM Head 对条件分布表达秩的限制。把 $$M$$ 个上下文隐藏状态堆成 $$H \in \mathbb{R}^{M \times d}$$，输出权重写成 $$W \in \mathbb{R}^{V \times d}$$，则 logits 矩阵 $$Z = HW^{\top}$$ 的秩至多为 $$d$$。逐行 LogSoftmax 只是再减去每行的归一化常数，因此模型可表达的 log-probability 矩阵秩至多约为 $$d+1$$。
...
\log p(j \mid i)
= h_i^{\top}w_j
  - \log \sum_k \exp\!\left(h_i^{\top}w_k\right)
$$

第一项组成低秩矩阵 $$HW^{\top}$$，第二项对同一行所有列相同，只增加至多一个秩一项。增加训练数据不能突破该结构上限，只能在受限函数族内找到更好解。
