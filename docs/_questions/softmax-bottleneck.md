...
1. **先给结论**：先定义它是输出分布矩阵的秩受隐藏维度限制，不是 Softmax 数值溢出。
2. **再讲关键机制**：用隐藏状态矩阵乘词向量矩阵说明 Logit 秩至多为 `d`，LogSoftmax 只多一个秩一项。
3. **主动说取舍**：增大词表不能提高秩上限；提高隐藏维度或 Mixture of Softmaxes 增容量也增加计算。
4. **最后落到项目**：通过验证困惑度、频次分桶、参数与吞吐消融判断是否真是瓶颈，随后停顿。

**60 秒口述示例：**

> 我的结论是 Softmax Bottleneck 是表达能力问题，不是指数溢出。若所有上下文隐藏状态组成矩阵 H、词表权重为 W，Logit 矩阵是 `HW^T`，秩不超过隐藏维度；LogSoftmax 的行归一化最多再引入一个秩一项，因此真实条件分布若更高秩，模型无法精确表示。扩大词表并不能解决。项目中我会先做隐藏维度或混合输出头消融，看困惑度和长尾词分桶是否改善，同时核算参数、吞吐和显存代价。
