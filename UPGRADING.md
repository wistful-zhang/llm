...
1. 在“面试记录”中导出 JSON；浏览器记录不在 GitHub 仓库里。
...
5. 合并后确认 GitHub Actions 全部绿色，并实际打开题库、模拟面试和面试记录页检查。

模板仓库与个人副本的 Git 历史彼此独立，直接 `git merge` 上游通常不适合新手。没有把握时，可以保留现有稳定版本，或让 Codex 在你的仓库中对照上游改动并保护上述内容目录。

## 仓库改名或转移 Owner

面试记录和练习进度会按仓库标识隔离。改名或转移前先导出面试记录 JSON；新网址上线后再恢复。公开网站地址改变时，也要检查 README、仓库 Website 和分享链接。
