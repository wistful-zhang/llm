...
test('v1 数据无损迁移到 v2，并把既有流程设为仅自己', () => {
  const ids = makeIds();
...
test('JSON 备份可跨仓库恢复并按 ID 合并，且保留公开意愿', () => {
  const ids = makeIds();
  const original = add(createEmptyTracker('old/repo'), { outcome: 'offer', visibility: 'public' }, ids);
...
test('CSV 导出防止表格公式注入，并保留中文字段与公开意愿', () => {
  ...
  assert.match(csv, /公开意愿/);
  assert.match(csv, /允许整理公开（不会自动上传）/);
