...
const generatedSource = '公开岗位面试主题整理；答案为面试口述稿，待逐题资料复核';
const batches = [batchA, batchB, batchC];
...
test('生成文件与批次 slug 精确一致，并保持统一的待逐题复核状态', async () => {
  ...
  const recordsByFilename = new Map(batchQuestions.map((record) => [
    `${generatedPrefix}${record.slug}.md`,
    record,
  ]));
  for (const { filename, parsed } of generatedDocuments) {
    const record = recordsByFilename.get(filename);
    assert.ok(record, `${filename} 必须对应一个批次 slug`);
