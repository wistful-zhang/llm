...
test('复杂 YAML 键、折叠标量、锚点和转义字符串不能制造解析歧义', () => {
  ...
  const literalBackslash = document().replace('title: "Transformer 为什么使用多头注意力？"', "title: '为什么 \\lambda 是技术题中的普通文本？'");
  ...
  assert.equal(parseQuestionDocument(literalBackslash).errors.length, 0);
});
