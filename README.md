...
一个可复用、可在线编辑、可自动发布的大模型面试准备与复盘项目。可以直接在网站 UI 中快速记题并保存为浏览器本机草稿，不要求 Markdown；需要同步时，再交给 Codex、GitHub 或 Pages CMS 写入仓库草稿，审核后才公开到阅读网站。网站还支持随机模拟、薄弱题复盘和仅保存在浏览器中的真实面试进度记录。
